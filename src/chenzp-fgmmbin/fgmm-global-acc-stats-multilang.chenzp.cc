...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "Usage:  fgmm-global-acc-stats [options] <model-in> <lang1-feature-rspecifier> [ <lang2-feature_rspecifier> ...] "
        "<stats-out>\n"
    ...
    std::string model_filename = po.GetArg(1),
        accs_wxfilename = po.GetArg(po.NumArgs());
    int32 num_lang = po.NumArgs() - 2;
    std::vector<std::string> feature_rspecifiers(num_lang);
    for (int k = 0; k < num_lang; k++) {
      feature_rspecifiers[k] = po.GetArg(k+2);
    }
