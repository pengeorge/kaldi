...
pthread_t jtid; // Current job thread ID
bool isOutOfMem;  // Whether current job is out of memory
...
void *funThreadJob(void *jobpara) {
...
void *funThreadMonitor(void *para) {
  while (1) {
    sleep(10);
    unsigned long total, free, buffer;
    double vm, resident;
    process_mem_usage(vm, resident); 
    get_raminfo(total, free, buffer);
    KALDI_VLOG(1) << "[I] Memory Usage: " << resident << " KB";
    KALDI_VLOG(1) << "[I] Total/Free/Buffer: " << total/1024 << " KB, " << free/1024 << " KB, " << buffer/1024 << " KB";
    unsigned long available = (free + buffer) / 1024;
    if (available < 28*1024*1024) {
      KALDI_VLOG(1) << "[I] Memory Usage: " << resident << " KB, only " << available << " KB available. Current job will be canceled.";
      isOutOfMem = true;
      pthread_cancel(jtid);
      /* void *res;
      pthread_join(jtid, &res);
      if (res != PTHREAD_CANCELED) {
          KALDI_VLOG(1) << "Why not canceled?! Force exiting...";
          exit(2);
      }*/
    }
  }
...
int main(int argc, char *argv[]) {
  try {
    ...
    if ((err = pthread_create(&mtid, NULL, funThreadMonitor, NULL))) {
      KALDI_ERR << "[E] Cannot create monitor thread. ";
      KALDI_ERR << "    Memory monitor will not work.";
    }
    ...
    for (; !keyword_reader.Done(); keyword_reader.Next()) {
      ...
      isOutOfMem = false;
      if ((err = pthread_create(&jtid, NULL, funThreadJob, &jobpara))) {
        KALDI_ERR << "[E] Cannot create job thread when processing " << jobpara.key << ", skip.";
        continue;
      }
      void *res;
      if ((err = pthread_join(jtid, &res))) {
        KALDI_ERR << "[E] pthread_join(" << jtid << ", NULL) failed, skip";
        continue;
      };
      if (isOutOfMem) {
        if (res != PTHREAD_CANCELED) {
          KALDI_ERR << "[E] out-of-memory Job was not canceled successfully. Exit process.";
          return -1;
        } else {
          KALDI_ERR << "[E] Out of memory when processing " << jobpara.key << ", killed.";
          continue;
        }
      } else if (res != 0) {
        KALDI_ERR << "[E] Exception caught when processing " << jobpara.key << ", skip.";
        continue;
      }
      n_done++;
    }
