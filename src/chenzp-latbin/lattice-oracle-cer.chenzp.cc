...
void SplitChars(fst::StdVectorFst *pfst, std::map<int, std::vector<int> > &w2c_map) {
  using namespace fst;
  typedef StdArc::StateId StateId;
  typedef StdArc::Label Label;
  for (StateIterator<StdVectorFst> siter(*pfst); !siter.Done(); siter.Next()) {
    StateId s = siter.Value();
    for (MutableArcIterator<StdVectorFst> aiter(pfst, s); !aiter.Done();  aiter.Next()) {
      StdArc arc = aiter.Value();
      StateId nextS = arc.nextstate;
      KALDI_ASSERT(arc.ilabel == arc.olabel);
      KALDI_ASSERT(w2c_map.find(arc.ilabel) != w2c_map.end());
      Label label = arc.ilabel;
      int splitArcNum = w2c_map[label].size();
      if (splitArcNum > 1) {
        StateId prevS = pfst->AddState();
        arc.nextstate = prevS;
        arc.ilabel = w2c_map[label][0];
        arc.olabel = arc.ilabel;
        aiter.SetValue(arc);
        //for (std::vector<int>::const_iterator iter = w2c_map[label].begin();
        //     iter != w2c_map[label].end();   ++iter) {
        for (int i = 1; i < splitArcNum; i++) {
          StateId newS;
          if (i == splitArcNum - 1) {
            newS = nextS;
          } else {
          ...
          Label lab = w2c_map[label][i];
          pfst->AddArc(prevS, StdArc(lab, lab, 0, newS));
          prevS = newS;
        }
      }
    }
...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        "Finds the path having the smallest edit-distance (character level) between two lattices.\n"
        "For efficiency put the smallest lattices first (for example reference strings).\n"
        "Usage: lattice-oracle [options] <word-to-char-idfile> <test-lattice-rspecifier> <reference-rspecifier> "
        "<transcriptions-wspecifier> [<edit-distance-wspecifier>]\n"
        " e.g.: lattice-oracle-cer data/lang/w2c.int ark:lat.1 'ark:sym2int.pl -f 2- data/lang/words.txt <data/test/text' ark,t:-\n";
    ...
    //po.Register("word-to-char-idfile", &word_to_char_idfile,
    //           "A file mapping each word to character sequence (in ID form).");
    ...
    std::string w2c_idfile = po.GetArg(1),
        lats_rspecifier = po.GetArg(2),
        reference_rspecifier = po.GetArg(3),
        transcriptions_wspecifier = po.GetArg(4),
        edit_distance_wspecifier = po.GetOptArg(5);
    
    std::ifstream w2c(w2c_idfile.c_str());
    if (!w2c.is_open()) {
      KALDI_ERR << "Word-to-character ID file cannot be opened.";
    }
    std::map<int, std::vector<int> > w2c_map;
    for (std::string line; getline(w2c, line); ) {
      std::istringstream split(line);
      int word_id;
      split >> word_id;
      for (int each_id; split >> each_id; w2c_map[word_id].push_back(each_id));
    }
    w2c.close();
    ...
    for (; !lattice_reader.Done(); lattice_reader.Next()) {
      ...
      SplitChars(&lattice_fst, w2c_map);
      ...
      SplitChars(&reference_fst, w2c_map);
      ...
      if (best_path.Start() == fst::kNoStateId) {
        ...
        KALDI_LOG << "%CER " << (100.*tot_errs) / num_words << " [ " << tot_errs
    ...
    KALDI_LOG << "Overall %CER " << (100.*tot_errs)/tot_words << " [ "
