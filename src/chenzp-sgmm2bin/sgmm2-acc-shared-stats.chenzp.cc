...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Accumulate stats (no transition accs) for SGMM training.\n"
        "Usage: sgmm2-acc-shared-stats [options] <model-in> <feature-rspecifier> "
    ...
    std::string update_flags_str = "MNwS";
    ...
    MleAmSgmm2AccsShared sgmm_accs(rand_prune);
