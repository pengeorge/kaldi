...
void MleAmSgmm2AccsShared::Write(std::ostream &out_stream, bool binary) const {

  WriteToken(out_stream, binary, "<SGMMACCS>");
  if (!single_gamma_) {
    WriteToken(out_stream, binary, "<NUMModels>");
    WriteBasicType(out_stream, binary, num_models_);
  }
  ...
  if (single_gamma_) {
    ...
    if (mdl_gammas_.size() != 0) { 
      KALDI_ASSERT(mdl_gammas_.size() == num_models_);
      KALDI_ASSERT(model_num_groups_.Dim() == num_models_);
      WriteToken(out_stream, binary, "<model_num_groups>");
      model_num_groups_.Write(out_stream, binary);
      WriteToken(out_stream, binary, "<gammas>");
      for (int32 l = 0; l < num_models_; l++) {
        for (int32 j1 = 0, max = model_num_groups_(l); j1 < max; j1++) {
          Matrix<BaseFloat> gamma_j1(mdl_gammas_[l][j1]);
          gamma_j1.Write(out_stream, binary);
        }
      }
    }
  ...
  if (gamma_i_.Dim() != 0) {
    WriteToken(out_stream, binary, "<gamma_i>");
    if (!single_gamma_) { // means we've got gamma_i_ by accumulation
      Vector<BaseFloat>(gamma_i_).Write(out_stream, binary);
    } else { // we have single gamma_, so compute gamma_i now
      Vector<double> gamma_i(num_gaussians_);
      for (int32 j1 = 0; j1 < num_groups_; j1++)
        gamma_i.AddRowSumMat(1.0, gamma_[j1]); // add sum of rows of
      // accs.gamma_[j1], to gamma_i.
      Vector<BaseFloat>(gamma_i).Write(out_stream, binary);
    }
  }
  if (t_.NumRows() != 0) {
  ...
  if (single_gamma_ && gamma_c_.size() != 0) {
...
/*void MleAmSgmm2AccsShared::SetNumAndDim(const AmSgmm2 &model) {
  // All nums and dims can be reset except num_gaussians_
...
void MleAmSgmm2AccsShared::Read(std::istream &in_stream, bool binary,
                         bool add) {
  ExpectToken(in_stream, binary, "<SGMMACCS>");
  string token;
  ReadToken(in_stream, binary, &token);
  if (token == "<NUMModels>") {
    ReadBasicType(in_stream, binary, &num_models_);
    KALDI_ASSERT(single_gamma_ == false);
    ExpectToken(in_stream, binary, "<NUMPDFS>");
  } else {
    KALDI_ASSERT(token == "<NUMPDFS>");
  }
  ...
  while (token != "</SGMMACCS>") {
    if (token == "<Y>") {
      ...
      KALDI_LOG << "Adding Y, num_gaussians = " << num_gaussians_;
      for (size_t i = 0; i < Y_.size(); i++) {
        Y_[i].Read(in_stream, binary, add);
      }
    } else if (token == "<Z>") {
      Z_.resize(num_gaussians_);
      KALDI_LOG << "Adding Z, num_gaussians = " << num_gaussians_;
      ...
      KALDI_LOG << "Adding R, num_gaussians = " << num_gaussians_;
      for (size_t i = 0; i < R_.size(); i++) {
        R_[i].Read(in_stream, binary, add);
      }
    } else if (token == "<S>") {
      S_.resize(num_gaussians_);
      KALDI_LOG << "Adding S, num_gaussians = " << num_gaussians_;
      for (size_t i = 0; i < S_.size(); i++) {
        S_[i].Read(in_stream, binary, add);
      }
   /* } else if (token == "<S_means>") {
      S_means_.resize(num_gaussians_);
      KALDI_LOG << "Adding S_means, num_gaussians = " << num_gaussians_;
      for (size_t i = 0; i < S_means_.size(); i++) {
        S_means_[i].Read(in_stream, binary, add);
      }*/
    } else if (token == "<y>") {
      y_.resize(num_groups_);
      KALDI_LOG << "Adding y, num_groups = " << num_groups_;
      for (int32 j1 = 0; j1 < num_groups_; j1++) {
        y_[j1].Read(in_stream, binary, add);
      }
    } else if (token == "<gamma_i>") {
      KALDI_LOG << "Adding gamma_i, num_gaussians = " << num_gaussians_;
      gamma_i_.Read(in_stream, binary, add);
    } else if (token == "<model_num_groups>") { // means the input acc has multiple gammas
      KALDI_ASSERT(single_gamma_ == false);
      if (model_num_groups_.Dim() == 0) {
        model_num_groups_.Resize(num_models_);
        model_num_groups_.Read(in_stream, binary, false);
        for (int32 l = 0; l < num_models_; l++) {
          KALDI_LOG << "model_num_groups_(" << l << ") = " << model_num_groups_(l);
        }
      } else {
        Vector<double> this_model_num_groups(num_models_);
        this_model_num_groups.Read(in_stream, binary, false);
        for (int32 l = 0; l < num_models_; l++) {
          KALDI_ASSERT(ApproxEqual(model_num_groups_(l), this_model_num_groups(l)));
        }
      }
    } else if (token == "<gammas>") {
      KALDI_ASSERT(single_gamma_ == false);
      KALDI_ASSERT(num_models_ == model_num_groups_.Dim());
      mdl_gammas_.resize(num_models_);
      for (int32 l = 0; l < num_models_; l++) {
        int32 ng = model_num_groups_(l);
        mdl_gammas_[l].resize(ng);
        for (int32 j1 = 0; j1 < ng; j1++) {
          mdl_gammas_[l][j1].Read(in_stream, binary, add);
        }
      }
    } else if (token == "<gamma>") { // means the input acc is single_gamma
      if (single_gamma_) {
        KALDI_LOG << "Adding gamma_, num_groups = " << num_groups_;
        ...
        if (!add) {
          ...
          model_num_groups_.Resize(0);
          mdl_gammas_.clear();
          num_models_ = 0;
        }
        int32 l = num_models_;
        if (mdl_gammas_.size() == num_models_ - 1) { // num_models_ and model_num_groups_ have been updated when reading a_
          l--;
          KALDI_ASSERT(model_num_groups_(l) == num_groups_);
        } else if (mdl_gammas_.size() == num_models_) {
          std::vector<int32> tmp_bak(num_models_); // Ugly !!!
          for (int32 k = 0; k < num_models_; k++) {
            tmp_bak[k] = model_num_groups_(k);
          }
          num_models_++;
          model_num_groups_.Resize(num_models_);
          for (int32 k = 0; k < num_models_ - 1; k++) {
            model_num_groups_(k) = tmp_bak[k];
          }
          model_num_groups_(num_models_-1) = num_groups_;
          KALDI_LOG << "Adding a new model, num_models = " << num_models_
                    << ", num_groups_ = " << num_groups_;
        } else {
          KALDI_ASSERT(false);
        }
        std::vector< Matrix<double> > this_gamma(num_groups_);
        for (int32 j1 = 0; j1 < num_groups_; j1++) {
          this_gamma[j1].Read(in_stream, binary, false);
        }
        mdl_gammas_.push_back(this_gamma);
        KALDI_LOG << "mdl_gammas size: " << mdl_gammas_.size() << ", " << mdl_gammas_[l].size() << ", " << mdl_gammas_[l][0].NumRows() << "x" << mdl_gammas_[l][0].NumCols();
      }
    } else if (token == "<as>") {
      KALDI_ASSERT(single_gamma_ == false);
      KALDI_ASSERT(num_models_ == model_num_groups_.Dim());
      mdl_as_.resize(num_models_);
      for (int32 l = 0; l < num_models_; l++) {
        int32 ng = model_num_groups_(l);
        mdl_as_[l].resize(ng);
        for (int32 j1 = 0; j1 < ng; j1++) {
          mdl_as_[l][j1].Read(in_stream, binary, add);
        }
      }
    } else if (token == "<a>") {
      if (single_gamma_) {
        a_.resize(num_groups_);
        KALDI_LOG << "Adding a, num_groups = " << num_groups_;
        for (int32 j1 = 0; j1 < num_groups_; j1++) {
        ...
        int32 l = num_models_;
        if (mdl_as_.size() == num_models_ - 1) { // num_models_ and model_num_groups_ have been updated when reading gamma_
          l--;
          KALDI_ASSERT(model_num_groups_(l) == num_groups_);
        } else if (mdl_as_.size() == num_models_) {
          std::vector<int32> tmp_bak(num_models_); // Ugly !!!
          for (int32 k = 0; k < num_models_; k++) {
            tmp_bak[k] = model_num_groups_(k);
          }
        ...
        std::vector< Matrix<double> > this_a(num_groups_);
        for (int32 j1 = 0; j1 < num_groups_; j1++) {
          this_a[j1].Read(in_stream, binary, false);
        }
        mdl_as_.push_back(this_a);
        KALDI_LOG << "mdl_as size: " << mdl_as_.size() << ", " << mdl_as_[l].size() << ", " << mdl_as_[l][0].NumRows() << "x" << mdl_as_[l][0].NumCols();
      }
    } else if (token == "<gamma_c>") {
      if (single_gamma_) {
        gamma_c_.resize(num_pdfs_);
        KALDI_LOG << "Adding gamma_c, num_pdfs = " << num_pdfs_;
        for (int32 j2 = 0; j2 < num_pdfs_; j2++) {
          gamma_c_[j2].Read(in_stream, binary, add);
        }
      } else {
        std::vector< Vector<double> > this_gamma_c;
        this_gamma_c.resize(num_pdfs_);
        KALDI_LOG << "Discarding gamma_c, num_pdfs = " << num_pdfs_;
        for (int32 j2 = 0; j2 < num_pdfs_; j2++) {
          this_gamma_c[j2].Read(in_stream, binary, add);
        }
      }
    } else if (token == "<t>") {
      KALDI_LOG << "Adding t";
      ...
      KALDI_LOG << "Adding U, num_gaussians = " << num_gaussians_;
      for (int32 i = 0; i < num_gaussians_; i++) {
...
void MleAmSgmm2AccsShared::Check(const AmSgmm2 &model,
  ...
  if (show_properties) {
    KALDI_LOG << "Sgmm2PdfModel: J1 = " << num_groups_ << ", J2 = "
              ...
              << num_gaussians_;
    KALDI_LOG << "Input Model: J1 = " << model.NumGroups() << ", J2 = "
              << model.NumPdfs() << ", D = " << model.FeatureDim() << ", S = "
              << model.PhoneSpaceDim() << ", T = " << model.SpkSpaceDim() << ", I = "
              << model.NumGauss();
  }
 ...
 /* if (S_means_.size() == 0) {
    debug_str << "S_means: no.  ";
  } else {
    bool S_means_nz = false;
    KALDI_ASSERT(S_means_.size() == static_cast<size_t>(num_gaussians_));
    for (int32 i = 0; i < num_gaussians_; i++) {
      KALDI_ASSERT(S_means_[i].NumRows() == feature_dim_);
      if (!S_means_nz && S_means_[i](0, 0) != 0) { S_means_nz = true; }
    }
    debug_str << "S_means: yes, " << string(S_means_nz ? "nonzero. " : "zero. ");
  }*/
  ...
  if (single_gamma_) {
    ...
    if (mdl_as_.size() == 0) {
      ...
      debug_str << "mdl_as: yes. ";
      KALDI_ASSERT(mdl_as_.size() == static_cast<size_t>(num_models_));
      KALDI_ASSERT(ApproxEqual(model_num_groups_.Dim(), (double)num_models_));
      for (int32 l = 0; l < num_models_; l++) {
        int32 ng = model_num_groups_(l);
        KALDI_ASSERT(mdl_as_[l].size() == static_cast<size_t>(ng));
        for (int32 j1 = 0; j1 < ng; j1++) {
          KALDI_ASSERT(mdl_as_[l][j1].NumCols() == num_gaussians_);
        }
      }
    }
  }
  ...
  if (single_gamma_) {
    ...
    if (mdl_gammas_.size() == 0) {
      debug_str << "mdl_gammas: no. ";
    } else {
      debug_str << "mdl_gammas: yes. ";
      KALDI_ASSERT(mdl_gammas_.size() == static_cast<size_t>(num_models_));
      KALDI_ASSERT(ApproxEqual(model_num_groups_.Dim(), (double)num_models_));
      for (int32 l = 0; l < num_models_; l++) {
        int32 ng = model_num_groups_(l);
        KALDI_ASSERT(mdl_gammas_[l].size() == static_cast<size_t>(ng));
        for (int32 j1 = 0; j1 < ng; j1++) {
          KALDI_ASSERT(mdl_gammas_[l][j1].NumCols() == num_gaussians_);
          tot_gamma += mdl_gammas_[l][j1].Sum();
        }
      }
    }
  }
  ...
  if (gamma_i_.Dim() == 0) {
    debug_str << "gamma_i: no.  ";
  } else {
    KALDI_ASSERT(gamma_i_.Dim() == num_gaussians_);
    double tot_gamma_i = gamma_i_.Sum();
    bool nz = (tot_gamma_i != 0.0);
    debug_str << "gamma_i: yes, " << string(nz ? "nonzero. " : "zero. ");
    debug_str << "tot_gamma: " << tot_gamma << ", tot_gamma_i: " << tot_gamma_i << ", tot_gamma_c: " << tot_gamma_c << ". ";
    if ((!gamma_.empty() || !mdl_gammas_.empty()) && !ApproxEqual(tot_gamma_i, tot_gamma))
      KALDI_WARN << "Counts from gamma (mdl_gammas) and gamma_i differ "
                 << tot_gamma << " vs. " << tot_gamma_i;
    if (!gamma_c_.empty() && !ApproxEqual(tot_gamma_i, tot_gamma_c))
      KALDI_WARN << "Counts from gamma_c and gamma_i differ "
                 << tot_gamma_c << " vs. " << tot_gamma_i;
  }
...
void MleAmSgmm2AccsShared::CheckShared(const std::vector<AmSgmm2 *> &models,
                          bool show_properties) const {
  KALDI_ASSERT(single_gamma_ == false);
  KALDI_ASSERT(models.size() == num_models_);
  if (show_properties) {
    KALDI_LOG << "Sgmm2PdfModel: D = " << feature_dim_ << ", S = "
              << phn_space_dim_ << ", T = " << spk_space_dim_ << ", I = "
              << num_gaussians_;
    for (int32 l = 0; l < num_models_; l++) {
      KALDI_LOG << "Sgmm2PdfModel[" << l << "]: J1 = " << model_num_groups_(l)
                << ", D = " << feature_dim_ << ", S = "
                << phn_space_dim_ << ", T = " << spk_space_dim_ << ", I = "
                << num_gaussians_;
      KALDI_LOG << "Input Model[" << l << "]: J1 = " << models[l]->NumGroups() << ", J2 = "
                << models[l]->NumPdfs() << ", D = " << models[l]->FeatureDim() << ", S = "
                << models[l]->PhoneSpaceDim() << ", T = " << models[l]->SpkSpaceDim() << ", I = "
                << models[l]->NumGauss();
    }
  }
  
  for (int32 l = 0; l < num_models_; l++) {
    //KALDI_ASSERT(num_pdfs_ == models[l]->NumPdfs() && num_pdfs_ > 0);
    KALDI_ASSERT(model_num_groups_(l) == models[l]->NumGroups() && model_num_groups_(l) > 0);
    KALDI_ASSERT(num_gaussians_ == models[l]->NumGauss() && num_gaussians_ > 0);
    KALDI_ASSERT(feature_dim_ == models[l]->FeatureDim() && feature_dim_ > 0);
    KALDI_ASSERT(phn_space_dim_ == models[l]->PhoneSpaceDim() && phn_space_dim_ > 0);
    KALDI_ASSERT(spk_space_dim_ == models[l]->SpkSpaceDim());
  }
 ...
 /* if (S_means_.size() == 0) {
  ...
  if (mdl_as_.size() == 0) {
    ...
    for (int32 l = 0; l < num_models_; l++) {
      ...
      KALDI_ASSERT(mdl_as_[l].size() == static_cast<size_t>(ng));
      for (int32 j1 = 0; j1 < ng; j1++) {
        KALDI_ASSERT(mdl_as_[l][j1].NumRows() == models[l]->NumSubstatesForGroup(j1)
                  && mdl_as_[l][j1].NumCols() == num_gaussians_);
        if (!nz && mdl_as_[l][j1].Sum() != 0) nz = true;
      }
      debug_str << "mdl_as[" << l << "]: yes, " << string(nz ? "nonzero. " : "zero. "); // TODO: take out "string"
    }
  }
  /*if (a_.size() == 0) {
    debug_str << "a: no.  ";
  ...
  if (gamma_i_.Dim() == 0) {
    ...
    bool nz = (tot_gamma_i != 0.0);
    debug_str << "gamma_i: yes, " << string(nz ? "nonzero. " : "zero. ");
    debug_str << "tot_gamma: " << tot_gamma << ", tot_gamma_i: " << tot_gamma_i << ". ";
    if ((!gamma_.empty() || !mdl_gammas_.empty()) && !ApproxEqual(tot_gamma_i, tot_gamma))
      KALDI_WARN << "Counts from gamma (mdl_gammas) and gamma_i differ "
                 << tot_gamma << " vs. " << tot_gamma_i;
  }
...
void MleAmSgmm2AccsShared::CheckTotalGamma() const {
  std::ostringstream debug_str;
  ...
  if (single_gamma_) {
    if (gamma_.size() == 0) {
      ...
      KALDI_ASSERT(gamma_.size() == static_cast<size_t>(num_groups_));
      for (int32 j1 = 0; j1 < num_groups_; j1++) {
        KALDI_ASSERT(gamma_[j1].NumCols() == num_gaussians_);
        tot_gamma += gamma_[j1].Sum();
      }
      bool nz = (tot_gamma != 0.0);
      if (!gamma_c_.empty())
  ...
  KALDI_LOG << "Subspace GMM model properties (in CheckTotalGamma): " << debug_str.str();
}

void MleAmSgmm2AccsShared::ResizeAccumulators(const AmSgmm2 &model,
  ...
  if (flags & (kSgmmPhoneVectors | kSgmmSubstateWeights | kSgmmTransitions)) {
    KALDI_ERR << "Cannot update language dependent parameters";
  }
  ...
  if (flags & kSgmmCovarianceMatrix) {
    ...
    /*S_means_.resize(num_gaussians_);
    for (int32 i = 0; i < num_gaussians_; i++) {
      S_means_[i].Resize(feature_dim_);
    }*/
  } else {
    S_.clear();
    //S_means_.clear();
  ...
  if (flags & (kSgmmPhoneVectors | kSgmmPhoneWeightProjections |
    ...
    } else { // Nothing to do with mdl_gammas_
    }
  } else {
    if (single_gamma_)
      gamma_.clear();
  ...
  if (flags & (kSgmmPhoneProjections | kSgmmPhoneWeightProjections
             | kSgmmSpeakerProjections | kSgmmSpeakerWeightProjections)) {
    gamma_i_.Resize(num_gaussians_);
  } else {
    gamma_i_.Resize(0);
  }
  ...
  if (single_gamma_) { // always set up gamma_c_; it's nominally for
...
BaseFloat MleAmSgmm2AccsShared::Accumulate(const AmSgmm2 &model,
...
BaseFloat MleAmSgmm2AccsShared::AccumulateFromPosteriors(
    const AmSgmm2 &model,
    const Sgmm2PerFrameDerivedVars &frame_vars,
...
void MleAmSgmm2AccsShared::CommitStatsForSpk(const AmSgmm2 &model,
...
void MleAmSgmm2AccsShared::GetStateOccupancies(Vector<BaseFloat> *occs) const {
...
void MleAmSgmm2SharedUpdater::Update(const MleAmSgmm2AccsShared &accs,
                               const std::vector<AmSgmm2 *> &models,
                               SgmmUpdateFlagsType flags) {
  KALDI_ASSERT(accs.single_gamma_ == false);
  KALDI_ASSERT(accs.num_models_ == models.size());
  ...
  // accs.gamma_[j1], to gamma_i.*/
  KALDI_ASSERT(accs.gamma_i_.Dim() == accs.num_gaussians_);

  if (flags & kSgmmPhoneProjections)
    ComputeQ(accs, models, &Q);

  if (flags & kSgmmCovarianceMatrix)
    ComputeSMeans(accs, models, &S_means);
  ...
  std::vector< SpMatrix<double> > H; //s(accs.num_models_);  H is computed by M and Sigma, should be the same in all models
  ...
  //if (flags & kSgmmPhoneVectors)
  //  tot_impr += UpdatePhoneVectors(accs, H, log_a, models);
  if (flags & kSgmmPhoneProjections) {
    if (options_.tau_map_M > 0.0) {
      for (int32 l = 1; l < accs.num_models_; l++) {
        if (models[l]->M_prior_.size() == 0 || models[l]->col_cov_inv_.NumRows() == 0
            || models[l]->row_cov_inv_.NumRows() == 0) {
          KALDI_LOG << "Computing the prior first for lang " << l;
          ComputeMPrior(models[l]);
        }
      }
      tot_impr += MapUpdateM(accs, Q, accs.gamma_i_, models[0]);  // MAP adaptation of M     assuming all models have the same M
    } else {
      tot_impr += UpdateM(accs, Q, accs.gamma_i_, models[0]); // assuming all models have the same M
    }
    for (int32 l = 1; l < accs.num_models_; l++) {
      for (int32 i = 0; i < accs.num_gaussians_; i++) {
        models[l]->M_[i].CopyFromMat(models[0]->M_[i]);
      }
    }
  }
  if (flags & kSgmmPhoneWeightProjections)
    tot_impr += UpdateW(accs, log_a, accs.gamma_i_, models);
  if (flags & kSgmmCovarianceMatrix)
    tot_impr += UpdateVars(accs, S_means, accs.gamma_i_, models);
  /*if (flags & kSgmmSubstateWeights)
    tot_impr += UpdateSubstateWeights(accs, model);*/
  if (flags & kSgmmSpeakerProjections) {
    tot_impr += UpdateN(accs, accs.gamma_i_, models[0]);
    for (int32 l = 0; l < accs.num_models_; l++) {
      for (int32 i = 0; i < accs.num_gaussians_; i++) {
        models[l]->N_[i].CopyFromMat(models[0]->N_[i]);
      }
    }
  }
  if (flags & kSgmmSpeakerWeightProjections) {
    tot_impr += UpdateU(accs, accs.gamma_i_, models[0]);
    for (int32 l = 0; l < accs.num_models_; l++) {
      for (int32 i = 0; i < accs.num_gaussians_; i++) {
        models[l]->u_.Row(i).CopyFromVec(models[0]->u_.Row(i));
      }
    }
  }
  if ((flags & kSgmmSpeakerProjections) && (options_.renormalize_N)) {
    RenormalizeN(accs, accs.gamma_i_, models[0]); // if you renormalize N you have to
  ...
  if (options_.renormalize_V) {
    for (int32 l = 0; l < accs.num_models_; l++) {
      RenormalizeV(accs, models[l], accs.gamma_i_, H);
    }
  }
  ...
  for (int32 l = 0; l < accs.num_models_; l++) {
    models[l]->n_.clear(); // has become invalid.
    models[l]->w_jmi_.clear(); // has become invalid.
  // we updated the v or w quantities.
  }
}

// Compute the Q_{i} (Eq. 64)
void MleAmSgmm2SharedUpdater::ComputeQ(const MleAmSgmm2AccsShared &accs,
                                const std::vector<AmSgmm2 *> &models,
                                std::vector< SpMatrix<double> > *Q) {
  ...
  for (int32 i = 0; i < accs.num_gaussians_; i++) {
    ...
    for (int32 l = 0; l < accs.num_models_; l++) {
      AmSgmm2 *p_this_model = models[l];
      int32 ng = accs.model_num_groups_(l);
      for (int32 j1 = 0; j1 < ng; j1++) {
        for (int32 m = 0; m < p_this_model->NumSubstatesForGroup(j1); m++) {
          if (accs.mdl_gammas_[l][j1](m, i) > 0.0) {
            (*Q)[i].AddVec2(static_cast<BaseFloat>(accs.mdl_gammas_[l][j1](m, i)),
                            p_this_model->v_[j1].Row(m));
          }
        }
      }
    }
...
void MleAmSgmm2SharedUpdater::ComputeSMeans(const MleAmSgmm2AccsShared &accs,
                                     const std::vector<AmSgmm2 *> &models,
                                     std::vector< SpMatrix<double> > *S_means) {
  ...
  for (int32 i = 0; i < accs.num_gaussians_; i++) {
    ...
    YM_MY.AddMatMat(-1.0, accs.Y_[i], kNoTrans,
                    Matrix<double>(models[0]->M_[i]), kTrans, 0.0);
    ...
    for (int32 l = 0; l < accs.num_models_; l++) {
      for (int32 j1 = 0; j1 < accs.model_num_groups_(l); j1++) {
        for (int32 m = 0; m < models[l]->NumSubstatesForGroup(j1); m++) {
          if (accs.mdl_gammas_[l][j1](m, i) != 0.0) {
            // Sigma_{i} += gamma_{jmi} * mu_{jmi}*mu_{jmi}^T
            mu_jmi.AddMatVec(1.0, models[l]->M_[i], kNoTrans, models[l]->v_[j1].Row(m), 0.0);
            (*S_means)[i].AddVec2(static_cast<BaseFloat>(accs.mdl_gammas_[l][j1](m, i)), mu_jmi);
          }
        }
      }
    }
    //(*S_means)[i].AddSp(1.0, accs.S_means_[i]);  // Sigma_{i} += S_means_{i}
...
class UpdatePhoneVectorsClass: public MultiThreadable { // For multi-threaded.
 public:
  UpdatePhoneVectorsClass(const MleAmSgmm2SharedUpdater &updater,
                          const MleAmSgmm2AccsShared &accs,
 ...
 private:
  const MleAmSgmm2SharedUpdater &updater_;
...
double MleAmSgmm2SharedUpdater::UpdatePhoneVectors(
    const MleAmSgmm2AccsShared &accs,    
...
//static
void MleAmSgmm2SharedUpdater::ComputeLogA(const MleAmSgmm2AccsShared &accs,
                                    std::vector<Matrix<double> > *log_a) {
...
void MleAmSgmm2SharedUpdater::UpdatePhoneVectorsInternal(
    const MleAmSgmm2AccsShared &accs,
    const vector< SpMatrix<double> > &H,
    const vector< Matrix<double> > &log_a,
    AmSgmm2 *model,
...
void MleAmSgmm2SharedUpdater::RenormalizeV(const MleAmSgmm2AccsShared &accs,
                                    AmSgmm2 *model,
...
double MleAmSgmm2SharedUpdater::UpdateM(const MleAmSgmm2AccsShared &accs,
...
// ICASSP 2012.
void MleAmSgmm2SharedUpdater::ComputeMPrior(AmSgmm2 *model) {
...
double MleAmSgmm2SharedUpdater::MapUpdateM(const MleAmSgmm2AccsShared &accs,
                                     const std::vector< SpMatrix<double> > &Q,
                                     const Vector<double> &gamma_i,
                                     AmSgmm2 *model) {
  int32 Ddim = model->FeatureDim();
...
// static
void MleAmSgmm2SharedUpdater::UpdateWGetStats(const MleAmSgmm2AccsShared &accs,
                                        int32 l,
                                        const AmSgmm2 &model,
  ...
  //KALDI_LOG << "Start thread " << thread_id << "/" << num_threads << " for lang " << l << ". num_groups=" << accs.model_num_groups_(l);
  // Accumulate stats from a block of states (this gets called in parallel).
  int32 block_size = ((int32)(accs.model_num_groups_(l)) + (num_threads-1)) / num_threads,
      j1_start = block_size * thread_id,
      j1_end = std::min((int32)(accs.model_num_groups_(l)), j1_start + block_size);
  //KALDI_LOG << "j1 from " << j1_start << " to " << j1_end << " : Thread " << thread_id << "/" << num_threads << " for lang " << l;
  ...
  for (int32 j1 = j1_start; j1 < j1_end; j1++) {
    int32 num_substates = accs.mdl_gammas_[l][j1].NumRows(); //model.NumSubstatesForGroup(j1);
    ...
    for (int32 m = 0; m < num_substates; m++) {
      ...
      double gamma_jm = accs.mdl_gammas_[l][j1].Row(m).Sum();
      w_jm.Add(-1.0 * w_jm.LogSumExp());
      *tot_like += VecVec(w_jm, accs.mdl_gammas_[l][j1].Row(m));
      w_jm.ApplyExp();
      v_vT.SetZero();
      ...
      for (int32 i = 0; i < accs.num_gaussians_; i++) {
        ...
        linear_term(m, i) = accs.mdl_gammas_[l][j1](m, i) - gamma_jm * w_jm(i);
        quadratic_term(m, i) = std::max(accs.mdl_gammas_[l][j1](m, i),
                                        gamma_jm * w_jm(i));
      }
    } // loop over substates
  ...
  //KALDI_LOG << "Exit thread " << thread_id << "/" << num_threads << " for lang " << l;
}

// The parallel weight update, in the paper.
double MleAmSgmm2SharedUpdater::UpdateW(const MleAmSgmm2AccsShared &accs,
                                  const std::vector<Matrix<double> > &log_a,
                                  const Vector<double> &gamma_i,
                                  const std::vector<AmSgmm2 *> &models) {
  KALDI_ASSERT(models.size() == accs.num_models_);
  KALDI_ASSERT(accs.num_models_ > 0 && accs.single_gamma_ == false);
  KALDI_LOG << "Updating weight projections using " << accs.num_models_ << " models";
  ...
  Matrix<double> w(models[0]->w_); // w should be the same in all models
  ...
  for (int iter = 0; iter < options_.weight_projections_iters; iter++) {
    ...
    for (int32 l = 0; l < accs.num_models_; l++) {
      //g_num_threads = 1;
      UpdateWClassShared c(accs, l, *models[l], w, log_a, &F_i, &g_i, &k_like_before);
      RunMultiThreaded(c);
    }
    ...
    for (step_size = 1.0; step_size >= min_step; step_size /= 2) {
      ...
      for (int32 l = 0; l < accs.num_models_; l++) {
        for (int32 j1 = 0, max = accs.model_num_groups_(l); j1 < max; j1++) {
          //int32 M = model->NumSubstatesForGroup(j1);
          int32 M = accs.mdl_gammas_[l][j1].NumRows();
          Matrix<double> w_j(M, accs.num_gaussians_);
          w_j.AddMatMat(1.0, Matrix<double>(models[l]->v_[j1]), kNoTrans,
                         w, kTrans, 0.0);
          if (!log_a.empty()) w_j.AddMat(1.0, log_a[j1]); // SSGMM techreport eq. 42
          for (int32 m = 0; m < M; m++) {
            SubVector<double> w_jm(w_j, m);
          ...
          k_like_after += TraceMatMat(w_j, accs.mdl_gammas_[l][j1], kTrans);
        }
      }
  ...
  for (int32 l = 0; l < accs.num_models_; l++) {
    models[l]->w_.CopyFromMat(w);
    models[l]->w_jmi_.clear(); // invalidated.
  }
...
double MleAmSgmm2SharedUpdater::UpdateU(const MleAmSgmm2AccsShared &accs,
                                 const Vector<double> &gamma_i,
                                 AmSgmm2 *model) {
  double tot_impr = 0.0;
  SolverOptions opts;
...
double MleAmSgmm2SharedUpdater::UpdateN(const MleAmSgmm2AccsShared &accs,
                                 const Vector<double>  &gamma_i,
                                 AmSgmm2 *model) {
  double tot_count = 0.0, tot_like_impr = 0.0;
...
void MleAmSgmm2SharedUpdater::RenormalizeN(const MleAmSgmm2AccsShared &accs,
                                    const Vector<double> &gamma_i,
                                    AmSgmm2 *model) {
...
double MleAmSgmm2SharedUpdater::UpdateVars(const MleAmSgmm2AccsShared &accs,
  ...
  for (int32 i = 0; i < accs.num_gaussians_; i++) {
    ...
    objf_improv(i) = models[0]->SigmaInv_[i].LogPosDefDet() -
        TraceSpSp(SpMatrix<double>(models[0]->SigmaInv_[i]), Sigma_i_ml);

    for (int32 l = 0; l < accs.num_models_; l++) {
      models[l]->SigmaInv_[i].CopyFromSp(Sigma_i_ml);  // inverted in the next loop.
    }
  }
  ...
  for (int32 i = 0; i < accs.num_gaussians_; i++) {
    Sigma_i.CopyFromSp(models[0]->SigmaInv_[i]);
    ...
    if (gamma_i(i) < options_.cov_diag_ratio * accs.feature_dim_) {
      ...
      for (int32 l = 0; l < accs.num_models_; l++) {
        models[l]->SigmaInv_[i].CopyFromSp(Sigma_i);
        models[l]->SigmaInv_[i].InvertDouble();
      }
      ...
      try {
        ...
        for (int32 l = 0; l < accs.num_models_; l++) {
          models[l]->SigmaInv_[i].SetUnit();  // Set to unit.
        }
      }
    }
  }
...
double MleAmSgmm2SharedUpdater::UpdateSubstateWeights(
    const MleAmSgmm2AccsShared &accs, AmSgmm2 *model) {
...
MleAmSgmm2AccsShared::~MleAmSgmm2AccsShared() {
  if (gamma_s_.Sum() != 0.0)
    KALDI_ERR << "In destructor of MleAmSgmm2AccsShared: detected that you forgot to "
