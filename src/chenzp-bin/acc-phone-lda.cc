...
/** @brief Accumulate LDA statistics based on phone-ids. Inputs are the
...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
        "Accumulate LDA statistics based on phone-ids.\n"
    ...
    for (;!feature_reader.Done(); feature_reader.Next()) {
      ...
      if (lda.Dim() == 0)
        lda.Init(trans_model.NumPhones(), feats.NumCols());
      ...
      Posterior phone_post;
      ConvertPosteriorToPhones(trans_model, post, &phone_post);
      for (int32 i = 0; i < feats.NumRows(); i++) {
        SubVector<BaseFloat> feat(feats, i);
        for (size_t j = 0; j < phone_post[i].size(); j++) {
          int32 phone_id = phone_post[i][j].first;
          BaseFloat weight = RandPrune(phone_post[i][j].second, rand_prune);
          if (weight != 0.0) {
            lda.Accumulate(feat, phone_id - 1, weight); // phone is one-based, so minus one here
          }
        }
      }
      num_done++;
