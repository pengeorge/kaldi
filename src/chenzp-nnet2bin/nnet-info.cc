...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "acoustic model to the standard output (no transition model, only NN)\n"
        "Usage:  nnet-info [options] <raw-nnet-in>\n"
        "e.g.:\n"
        " nnet-info 1.raw\n";
    ...
    if (print_learning_rates) {
      Vector<BaseFloat> learning_rates(nnet.NumUpdatableComponents());
      nnet.GetLearningRates(&learning_rates);
      int32 nc = learning_rates.Dim();
      for (int32 i = 0; i < nc; i++)
        std::cout << learning_rates(i) << (i < nc - 1 ? ":" : "");
      std::cout << std::endl;
      KALDI_LOG << "Printed learning-rate info for " << raw_nnet_rxfilename;
    } else {
      std::cout << nnet.Info();
