...
int main(int argc, char *argv[]) {
  try {
    ...
    const char *usage =
        ...
        "\n"
        "Usage:  nnet-insert-and-resize [options] <nnet-in> <raw-nnet-to-insert-in> <nnet-out>\n"
        "e.g.:\n"
    ...
    InsertComponentsAndResize(src_nnet,
                     insert_at,
                     &(am_nnet.GetNnet()));
    ...
    if (randomize_next_component) {
      ...
      if (!ac)
        KALDI_ERR << "The component before softmax is not an AffineComponent: "
                  << component->Info();
