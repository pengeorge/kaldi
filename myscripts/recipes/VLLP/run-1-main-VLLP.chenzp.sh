...
#Preparing tun3h and train directories
train_data_dir=`dirname $train_data_trans_dir`
if [ ! -f data/raw_train_data/.done ]; then
  echo ---------------------------------------------------------------------
  echo "Subsetting the TRAIN set"
  echo ---------------------------------------------------------------------

  echo 'train_data_dir is '$train_data_dir
  if [ ! -d $train_data_dir/audio ]; then
    ln -sf $train_data_audio_dir $train_data_dir/audio
  fi
  if [ ! -d $train_data_dir/transcription ]; then
    ln -sf `basename $train_data_trans_dir` $train_data_dir/transcription
  fi
  if [ ! -v train_data_list ] || [ -z "$train_data_list" ]; then
    if [ ! -z $train_ecf_file ]; then
      grep -Po '(?<=audio_filename=")[^"]*(?=")' $train_ecf_file |sort -u > $train_data_dir/train.list
      train_data_list=$train_data_dir/train.list
    else
      echo "ERROR: Both train_data_dir and train_ecf_file are empty."
      exit 1
    fi
  fi
...
tun3h_data_dir=`dirname $tun3h_data_trans_dir`
if [ ! -d data/raw_tun3h_data ]; then
  echo ---------------------------------------------------------------------
  echo "Subsetting the TUN3H set"
  echo ---------------------------------------------------------------------  
  if [ ! -d $tun3h_data_dir/audio ]; then
    ln -sf $tun3h_data_audio_dir $tun3h_data_dir/audio
  fi
  if [ ! -d $tun3h_data_dir/transcription ]; then
    ln -sf `basename $tun3h_data_trans_dir` $tun3h_data_dir/transcription
  fi
  if [ -z $tun3h_data_list ]; then
    if [ ! -z $tun3h_ecf_file ]; then
      grep -Po '(?<=audio_filename=")[^"]*(?=")' $tun3h_ecf_file |sort -u > $tun3h_data_dir/tun3h.list
      tun3h_data_list=$tun3h_data_dir/tun3h.list
    else
      echo "ERROR: Both tun3h_data_dir and tun3h_ecf_file are empty."
      exit 1
    fi
  fi
  local/make_corpus_subset.sh "$tun3h_data_dir" "$tun3h_data_list" ./data/raw_tun3h_data || exit 1
else
  if [ -z $tun3h_data_list ]; then
    if [ ! -z $tun3h_ecf_file ]; then
      grep -Po '(?<=audio_filename=")[^"]*(?=")' $tun3h_ecf_file |sort -u > $tun3h_data_dir/tun3h.list
...
dev10h_data_dir=`dirname $dev10h_data_trans_dir`
if [ ! -d data/raw_dev10h_data ]; then
  echo ---------------------------------------------------------------------
  echo "Subsetting the DEV10H set"
  echo ---------------------------------------------------------------------  
  if [ ! -d $dev10h_data_dir/audio ]; then
    ln -s $dev10h_data_audio_dir $dev10h_data_dir/audio
  fi
  if [ ! -d $dev10h_data_dir/transcription ]; then
    ln -s `basename $dev10h_data_trans_dir` $dev10h_data_dir/transcription
  fi
  if [ -z $dev10h_data_list ]; then
    if [ ! -z $dev10h_ecf_file ]; then
      grep -Po '(?<=audio_filename=")[^"]*(?=")' $dev10h_ecf_file |sort -u > $dev10h_data_dir/dev10h.list
      dev10h_data_list=$dev10h_data_dir/dev10h.list
    else
      echo "ERROR: Both dev10h_data_dir and dev10h_ecf_file are empty."
      exit 1
    fi
  fi
...
nj_max=`cat $tun3h_data_list | wc -l`
...
if [[ ! -f data/local/lexicon.txt || data/local/lexicon.txt -ot "$lexicon_file" ]]; then
  ...
  echo ---------------------------------------------------------------------
  czpScripts/local/make_lexicon_subset.chenzp.sh --case-insensitive $case_insensitive $train_data_dir/transcription $lexicon_file data/local/filtered_lexicon.txt
  czpScripts/local/prepare_lexicon.chenzp.pl  --phonemap "$phoneme_mapping" \
    --stress-tag "$phone_stress_tag" \
...
if $lang_only; then
  echo "Exiting after prepare_lang, as requested. "
...
if [[ ! -f data/train/wav.scp || data/train/wav.scp -ot "$train_data_dir" ]]; then
  ...
  echo ---------------------------------------------------------------------
  mkdir -p data/train
  czpScripts/local/prepare_acoustic_training_data.chenzp.pl \
    --case-insensitive $case_insensitive \
    --vocab data/local/lexicon.txt --fragmentMarkers \-\*\~ \
    $train_data_dir data/train > data/train/skipped_utts.log
fi

if [[ ! -f data/tun3h/wav.scp || data/tun3h/wav.scp -ot ./data/raw_tun3h_data/audio ]]; then
  echo ---------------------------------------------------------------------
  echo "Preparing tun3h data lists in data/tun3h on" `date`
  echo ---------------------------------------------------------------------
  mkdir -p data/tun3h
  czpScripts/local/prepare_acoustic_training_data.chenzp.pl \
    --case-insensitive $case_insensitive \
    --vocab data/local/lexicon.txt --fragmentMarkers \-\*\~ \
    --fragmentMarkers \-\*\~ \
    `pwd`/data/raw_tun3h_data data/tun3h > data/tun3h/skipped_utts.log || exit 1
  mkdir -p data/tun3h_noUnk
  czpScripts/local/prepare_acoustic_training_data.chenzp.pl \
    --case-insensitive $case_insensitive \
    --fragmentMarkers \-\*\~ \
    `pwd`/data/raw_tun3h_data data/tun3h_noUnk > data/tun3h_noUnk/skipped_utts.log || exit 1
fi

if [[ ! -f data/tun3h/glm || data/tun3h/glm -ot "$glmFile" ]]; then
  echo ---------------------------------------------------------------------
  echo "Preparing tun3h stm files in data/tun3h on" `date`
  echo ---------------------------------------------------------------------
  if [ -z $tun3h_stm_file ]; then 
    ...
    local/prepare_stm.pl --fragmentMarkers \-\*\~ data/tun3h || exit 1
  else
    local/augment_original_stm.pl $tun3h_stm_file data/tun3h || exit 1
  fi
  [ ! -z $glmFile ] && cp $glmFile data/tun3h/glm
...
if [[ ! -f data/srilm/lm.gz || data/srilm/lm.gz -ot data/train/text ]]; then
  ...
  echo ---------------------------------------------------------------------
  if $case_insensitive; then # useless!!! text_for_lm_dev is the same as text
    cut -d' ' -f2- data/tun3h_noUnk/text | uconv -f utf8 -t utf8 -x "Any-Lower" |\
      paste -d' ' <(cut -d' ' -f1 data/tun3h_noUnk/text) - > data/tun3h_noUnk/text_for_lm_dev
  else
    ln -sf text data/tun3h_noUnk/text_for_lm_dev
  fi
  local/train_lms_srilm.sh --dev-text data/tun3h_noUnk/text_for_lm_dev \
...
if [ ! -f exp/sgmm5_mmi_b0.1/.done ]; then
  ...
  echo ---------------------------------------------------------------------
  czpScripts/steps/train_mmi_sgmm2.chenzp.sh \
