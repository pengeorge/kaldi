...
Usage:
       $0  <ecf_file> <kwlist-file> [rttm-file] <w2s-lex> <wlang-dir> <lang-dir> <data-dir>
allowed switches:
...
local/kws_data_prep_subword.chenzp.sh --case-insensitive ${case_insensitive} \
  $sil_opt --use_icu ${use_icu} --icu-transform "${icu_transform}" \
  $w2slex \
  $wlangdir $langdir $datadir $kwsdatadir || exit 1
