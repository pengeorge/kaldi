...
nj=30
lmwt=15  # used in lattice-amrescore for picking shortest paths
...
# AM rescore
s2s_suffix=${dev}_${count_cutoff}_beam${beam}
s2sdir=${decodedir}/s2s_${s2s_suffix}
if [ ! -f $s2sdir/.done ]; then
  if [ -f $s2sdir/.running ]; then
    echo "Waiting for s2s generated by another instance of this script..."
    while [ -f $s2sdir/.running ]; do
      sleep 60
    done
    if [ -f $s2sdir/.fail ]; then
      echo "The instance of this script generating s2s failed."
      exit 1;
    fi
    if [ ! -f $s2sdir/.done ]; then
      echo "Cannot find .running/.fail/.done, what happened..."
      exit 1;
    fi
    echo "s2s generated."
  else
    ...
    $decode_cmd JOB=1:$eval_nj $s2sdir/log/s2s.JOB.log \
      lattice-amrescore --s2s-only=true --confused-path-beam=$beam \
        "ark:gzip -cdf $decodedir/lat.JOB.gz |" \
        $cmdir/E.fst $L_dir/L.fst "ark:|gzip -c > $s2sdir/s2s.JOB.fsts.gz" || { touch $s2sdir/.fail; rm $s2sdir/.running; exit 1; }
    touch $s2sdir/.done
    rm $s2sdir/.running
  fi
fi
...
outdir=${decodedir}_amrescore_lm${lmwt}_${n}_${s2s_suffix}${suffix}
if [ ! -f $outdir/.done ]; then
  eval_nj=`cat $decodedir/num_jobs`
  # beam, E and L are not used here
  $decode_cmd JOB=1:$eval_nj $outdir/log/amrescore.JOB.log \
    lattice-amrescore --s2s-rxfilename="ark:gzip -cdf $s2sdir/s2s.JOB.fsts.gz |" \
      --acoustic-scale=$acwt --n=$n "ark:gzip -cdf $decodedir/lat.JOB.gz |" \
