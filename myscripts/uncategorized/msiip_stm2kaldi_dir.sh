...
# Creating a UEM decoding setup with segmentation infomation in stm file.
dummy_text=true
text=
filelist=
...
[ -f ./path.sh ] && . ./path.sh
[ -f ./cmd_leave1q.sh ]  && . ./cmd_leave1q.sh
. parse_options.sh || exit 1;

if [ $# -ne 3 ] ; then
  echo "$0: Converts the CMU segmentation stm file into a kaldi data directory for UEM decoding"
  echo ""
  echo "cmu_ume2kaldi_dir.sh <stm> <path-to-sph-files> <output-data-dir>"
...
[ ! -f $stm ] && echo "STM file $1 does not exist!"  && exit 1;

echo "Converting `basename $stm` to kaldi directory $datadir "
grep -v ';;' $stm \
  | awk 'NF>=6{
           spk=$1;
           sub(/_inLine/,"_A",spk);
  ...
  | sort -u > $datadir/segments
#cat $stm | perl -pe 's:.+(BABEL):BABEL:; s:\}\s+\{FROM\s+: :; s:\}\s+\{TO\s+: :; s:\}.+::;' | \
#  perl -ne '@K = split; 
#            $utteranceID = @K[0]; 
