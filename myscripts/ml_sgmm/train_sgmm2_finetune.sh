...
if [ $# != 7 ]; then
  echo "Usage: steps/train_sgmm2_finetune.sh <num-leaves> <num-substates> <data> <lang> <ali-dir> <input-sgmm> <exp-dir>"
  echo " e.g.: steps/train_sgmm2.sh 5000 8000 data/train_si84 data/lang \\"
  echo "                      exp/tri3b_ali_si84 exp/sgmm4/final.mdl exp/sgmm4a"
...
# Check some files.
for f in $data/feats.scp $lang/L.fst $alidir/ali.1.gz $alidir/final.mdl $input_sgmm; do
...
if [ $stage -le -4 ]; then
  ...
  $cmd $dir/log/init_sgmm.log \
    sgmm2-init.chenzp --spk-dep-weights=$spk_dep_weights --self-weight=$self_weight \
       --pdf-map=$dir/pdf2group.map --phn-space-dim=$phn_dim \
       --init-from-sgmm=true \
       --spk-space-dim=$spk_dim $lang/topo $dir/tree $input_sgmm $dir/0.mdl || exit 1;
fi
...
while [ $x -lt $num_iters ]; do
   ...
   if [ $stage -le $x ]; then
     ...
     rm $dir/$x.*.acc $dir/$x.occs 2>/dev/null
   fi
