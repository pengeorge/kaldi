...
Usage: utils/write_kwslist.pl [options] <raw_result_in|-> <kwslist_out|->
 e.g.: utils/write_kwslist.pl --flen=0.01 --duration=1000 --segments=data/eval/segments
                              --normalize=kaldi --map-utter=data/kws/utter_map raw_results kwslist.xml
...
Allowed options:
  ...
  --normalize                 : Normalization method (kaldi/KST/skip)       (string,  default = kaldi)
...
my @norm_methods = ('skip', 'kaldi', 'KST', 'mix');
...
while (<$source>) {
  ...
  push(@KWS, [$kwid, $utter, $chnl, $start, $dur, $score, ""]);
}
