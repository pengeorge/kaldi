...
#  cat $langdir/words.txt | perl -e '
#  require "czpScripts/utils/libCase.pl";
#  while (<>) {
#      print &tolower($_);
#  }' > $kwsdatadir/words.txt
  ...
  paste <(cut -f 1  $kwsdatadir/keywords.txt  ) \
    ...
    czpScripts/kws/kwords2indices.chenzp.pl --map-oov 0  $kwsdatadir/words.txt > $kwsdatadir/keywords_all.int
else
...
(cut -f 1 -d ' ' $kwsdatadir/keywords.int | \
  czpScripts/kws/subset_kwslist.chenzp.pl $keywords > $kwsdatadir/kwlist_invocab.xml) || true

(cat $kwsdatadir/keywords_all.int | \
  egrep " 0 | 0$" | cut -f 1 -d ' ' | \
  czpScripts/kws/subset_kwslist.chenzp.pl $keywords > $kwsdatadir/kwlist_outvocab.xml) || true

# add by chenzp (Dec 20, 2013)
cat $kwsdatadir/keywords.int | \
  grep -P '^\S+ \d+$' > $kwsdatadir/keywords_invocab_1gram.int || true
#cat $kwsdatadir/keywords.int | \
#  grep -P '^\S+ \d+ \d+$' > $kwsdatadir/keywords_invocab_2gram.int
#cat $kwsdatadir/keywords.int | \
#  grep -P '^\S+ \d+ \d+ \d+$' > $kwsdatadir/keywords_invocab_3gram.int
cat $kwsdatadir/keywords.int | \
  grep -P '^\S+ \d+( \d+)+$' > $kwsdatadir/keywords_invocab_mgram.int
cat $kwsdatadir/keywords_all.int | egrep " 0 | 0$" > $kwsdatadir/keywords_oov.int
cat $kwsdatadir/keywords_oov.int | \
  grep -P '^\S+ \d+$' > $kwsdatadir/keywords_outvocab_1gram.int || true
#cat $kwsdatadir/keywords_oov.int | \
#  grep -P '^\S+ \d+ \d+$' > $kwsdatadir/keywords_outvocab_2gram.int
#cat $kwsdatadir/keywords_oov.int | \
#  grep -P '^\S+ \d+ \d+ \d+$' > $kwsdatadir/keywords_outvocab_3gram.int
cat $kwsdatadir/keywords_oov.int | \
  grep -P '^\S+ \d+( \d+)+$' > $kwsdatadir/keywords_outvocab_mgram.int

for suf in _{out,in}vocab_{1,m}gram; do
  (cat $kwsdatadir/keywords$suf.int | cut -f 1 -d ' ' | \
    czpScripts/kws/subset_kwslist.chenzp.pl $keywords > $kwsdatadir/kwlist$suf.xml) || true
done
...
if [ -z $silence_word ]; then
  ...
  for suf in _invocab_{1,m}gram; do 
    transcripts-to-fsts ark:$kwsdatadir/keywords$suf.int ark,t:$kwsdatadir/keywords$suf.fsts
  done
else
  ...
  for suf in _invocab_{1,m}gram; do 
    transcripts-to-fsts ark:$kwsdatadir/keywords$suf.int ark,t:- | \
      awk -v 'OFS=\t' -v silint=$silence_int '{if (NF == 4 && $1 != 0) { print $1, $1, silint, silint; } print; }' \
       > $kwsdatadir/keywords$suf.fsts
  done
fi
