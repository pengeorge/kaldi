...
if [ -z "$feat_type" ]; then
  if [ -f $nnetdir/feat_type ]; then
    feat_type=`cat $nnetdir/feat_type`
  elif [ -f $nnetdir/final.mat ]; then
    feat_type=lda
  else
    feat_type=delta
  fi
  ...
  if [ -f $nnetdir/feat_type ]; then
    if [ `cat $nnetdir/feat_type` != "$feat_type" ]; then
      echo "$0: feat_type not match, $nnetdir/feat_type is `cat $nnetdir/feat_type`, but you specify as $feat_type"
      exit 1;
    fi
  fi
fi
