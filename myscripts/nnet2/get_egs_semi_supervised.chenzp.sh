...
num_leaves=`tree-info $alidir/tree 2>/dev/null | awk '{print $2}' | sed -n '1p'` || exit 1;
...
# Write feat_dim and feat_type. This is the only difference from get_egs.sh (chenzp, Mar 25,2015)
feats_one="$(echo "$feats_sup" | sed s:JOB:1:g)"
feat_dim=$(feat-to-dim "$feats_one" -) || exit 1;
if [ ! -f $dir/feat_dim ]; then
  echo $feat_dim > $dir/feat_dim
fi
if [ ! -f $dir/feat_type ]; then
...
if [ $stage -le 3 ]; then
  ...
  $cmd $io_opts JOB=1:$nj_unsup $dir/log/get_weighted_egs.JOB.log \
    nnet-get-weighted-egs.chenzp $nnet_context_opts "${spk_vecs_opt_unsup[@]}" \
