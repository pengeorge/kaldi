...
num_hidden_layers=`nnet-am-info $input_model 2>/dev/null | grep TanhComponent | wc -l`
if [ `nnet-am-info final.mdl 2>/dev/null|grep 'component 1 : FixedAffineComponent' | wc -l` -eq 1 ]; then
  do_lda=true
else
  do_lda=false
fi
...
first_modify_iter=$[$finish_add_layers_iter + 2] # add_layers_period is typically 2
