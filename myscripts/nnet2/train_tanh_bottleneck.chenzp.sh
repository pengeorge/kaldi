...
EOF
  next_input_dim=$lda_dim
else
  next_input_dim=`perl -e "print (( $splice_width *2+1) * $feat_dim );"`
fi
cat >>$dir/nnet.config <<EOF
AffineComponentPreconditioned input-dim=$next_input_dim output-dim=$hidden_layer_dim alpha=$alpha max-change=$max_change learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
