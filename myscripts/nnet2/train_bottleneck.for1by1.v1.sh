...
if [ -z $unilingual_egs_dir ]; then
  echo "Uni-lingual egs dir should be ready before calling $0"
  exit 1;
fi

if [ $feat_const_dim -eq 0 ]; then
  egs_dir=$unilingual_egs_dir/egs
  feat_dim=`cat $unilingual_egs_dir/feat_dim` || exit 1;
fi
...
EOF
  ...
  if [ ! -z $lda_mat ]; then
    cat >>$dir/nnet.config <<EOF
FixedAffineComponentExt matrix=$lda_mat const-component-dim=0
EOF
    next_trans_input_dim=$lda_dim
  else
...
this_egs_dir=$unilingual_egs_dir/egs
x=0
while [ $x -lt $num_iters ]; do
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    # Set off jobs doing some diagnostics, in the background.
    $cmd $dir/log/compute_prob_valid.$x.log \
      nnet-compute-prob $dir/$x.mdl ark:$this_egs_dir/valid_diagnostic.egs &
    $cmd $dir/log/compute_prob_train.$x.log \
      nnet-compute-prob $dir/$x.mdl ark:$this_egs_dir/train_diagnostic.egs &
    if [ $x -gt 0 ] && [ ! -f $dir/log/mix_up.$[$x-1].log ]; then
      $cmd $dir/log/progress.$x.log \
        nnet-show-progress --use-gpu=no $dir/$[$x-1].mdl $dir/$x.mdl ark:$this_egs_dir/train_diagnostic.egs &
    fi
    ...
    if [ $x -gt 0 ] && \
      ...
      if [ $[($x-1) / $add_layers_period] -eq $[($num_hidden_layers-2)] ]; then
        ...
        if [ $feat_const_dim -gt 0 ]; then
          $cmd $dir/log/extend_const_component.$x.log \
            nnet-extend-const-component --const-dim=$feat_const_dim $dir/$x.mdl $dir/${x}ext.mdl \
            || exit 1;
          this_egs_dir=$egs_dir
        else
          cp $dir/$x.mdl $dir/${x}ext.mdl
        fi
        mdl="nnet-init --srand=$x $dir/bnf.config - | nnet-insert-and-resize $dir/${x}ext.mdl - - |"  
      else
    ...
    $cmd $parallel_opts JOB=1:$num_jobs_nnet $dir/log/train.$x.JOB.log \
      ...
      ark:$this_egs_dir/egs.JOB.$[$x%$iters_per_epoch].ark ark:- \| \
    ...
    if $shrink && [ $[$x % $shrink_interval] -eq 0 ]; then
      ...
      $cmd $parallel_opts $dir/log/shrink.$x.log \
        nnet-subset-egs --n=$num_frames_shrink --randomize-order=true --srand=$x \
          ark:$this_egs_dir/train_diagnostic.egs ark:-  \| \
