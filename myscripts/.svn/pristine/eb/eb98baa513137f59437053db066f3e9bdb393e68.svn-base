...
cmd=run.pl
num_epochs=15      # Number of epochs of training (for first language);
...
# For model initialization from scratch
...
# End of configuration for model initialization from scratch
...
if [ $# -lt 6 -o $[$#%2] -ne 0 ]; then
  ...
  echo "Usage: $0 [opts] <ali0> <egs0> <ali1> <egs1> ... <aliN-1> <egsN-1> <lang-dir0> <exp-dir>"
  echo " e.g.: $0 exp/tri6_ali exp/tri6_egs exp_lang2/tri6_ali exp_lang2/tri6_egs data/lang exp/tri6_multilang"
  echo ""
...
[ -z $alidir0_pdfs ] && echo "\$alidir0_pdfs (num_leaves) is unset" && exit 1
[ "$alidir0_pdfs" -eq "0" ] && echo "\$alidir0_pdfs (num_leaves) is 0" && exit 1
...
truncate_comp_num=$[2*$num_hidden_layers] # If do LDA (FixedAffine) at first, this should +1

# Using lang 0 to initialize the model
set -u

feat_dim=$(cat ${egs_dir[0]}/feat_dim) || exit 1;
...
AffineComponentPreconditioned input-dim=$splice_output_dim output-dim=$hidden_layer_dim alpha=$alpha max-change=$max_change learning-rate=$initial_learning_rate param-stddev=$stddev bias-stddev=$bias_stddev
TanhComponent dim=$hidden_layer_dim
AffineComponentPreconditioned input-dim=$hidden_layer_dim output-dim=$alidir0_pdfs alpha=$alpha max-change=$max_change learning-rate=$initial_learning_rate param-stddev=0 bias-stddev=0
SoftmaxComponent dim=$alidir0_pdfs
EOF
...
EOF
  $cmd $dir/log/nnet_init.log \
    nnet-am-init ${alidir[0]}/tree $langdir0/topo "nnet-init $dir/nnet.config -|" \
    $dir/pseudo_input0.mdl || exit 1;
fi

input_model=$dir/pseudo_input0.mdl
...
if [ ${num_jobs_nnet_array[0]} -gt $num_archives_expanded0 ]; then
  echo "$0: --num-jobs-nnet[0] ${num_jobs_nnet_array[0]} cannot exceed num-archives*frames-per-eg which is $num_archives_expanded0"
  exit 1;
fi
...
while [ $x -lt $num_iters ]; do
  ...
  if [ $x -ge 0 ] && [ $stage -le $x ]; then
    ...
    if [ $x -gt 0 ] && \
      ...
      if [ $[($x-1) / $add_layers_period] -eq $bottleneck_layer_num ]; then
        echo bnf layer with x = $x
        for lang in $(seq 0 $[$num_lang-1]); do
          mdls[$lang]="nnet-init --srand=$x $dir/bnf.config - | nnet-insert $dir/$lang/$x.mdl - - |"
        done
      else
        for lang in $(seq 0 $[$num_lang-1]); do
          mdls[$lang]="nnet-init --srand=$x $dir/hidden.config - | nnet-insert $dir/$lang/$x.mdl - - |"
        done
      fi
    else
      for lang in $(seq 0 $[$num_lang-1]); do
        mdls[$lang]=$dir/$lang/$x.mdl
      done
    fi

    if [ $x -eq 0 ] || [ "${mdls[0]}" != "$dir/0/$x.mdl" ]; then
      # on iteration zero or when we just added a layer, use a smaller minibatch size and only one quarter of the
      ...
      for lang in $(seq 0 $[$num_lang-1]); do
        ...
        for n in $(seq $this_num_jobs_nnet); do
          ...
          $cmd $parallel_opts $dir/$lang/log/train.$x.$n.log \
            ...
            --minibatch-size=$this_minibatch_size --srand=$x "${mdls[$lang]}" \
...
for lang in $(seq 0 $[$num_lang-1]); do
  if [ ! -f $dir/$lang/final.mdl ]; then
    echo "$0: $dir/$lang/final.mdl does not exist."
...
if [ -f $dir/0/final.mdl ]; then
  nnet-to-raw-nnet --truncate=$truncate_comp_num $dir/0/final.mdl $dir/final.raw
else 
