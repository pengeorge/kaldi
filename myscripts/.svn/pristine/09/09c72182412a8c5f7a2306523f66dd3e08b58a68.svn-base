# Author: chenzp
# IV expansion using reverse confusion matrix
...
function setup_proxy_search {
  local reverse_confusion_matrix=true
  local nbest=500
  local g2p_nbest=10
  local g2p_mass=0.95
  local beam=5
  ...
  local proxy_log_weight=`perl -e "print -log($proxy_weight);"`
  local original_log_weight=`perl -e "print -log($original_weight);"`
  # we permit the sum of proxy weight and original weight > 1.0
  local total_log_weight=`perl -e "print -log($original_weight + $proxy_weight);"`
  ...
  local kwsdatadir=${source_dir}_ive3
  mkdir -p $kwsdatadir
  ...
  if [ ! -f exp/conf_matrix_hyp2ref/.done ] ; then
    local/generate_confusion_matrix.chenzp.sh --reverse true --cmd "$decode_cmd" --nj $my_nj  \
      exp/sgmm5/graph exp/sgmm5 exp/sgmm5_ali exp/sgmm5_denlats  exp/conf_matrix_hyp2ref
    touch exp/conf_matrix_hyp2ref/.done 
  fi
  confusion=exp/conf_matrix_hyp2ref/confusions.txt
...
#    grep -v -f <(cut -f 1 $data_dir/${extraid}oov_kws/keywords.txt) $data_dir/${extraid}kws/keywords.txt | cut -f 1 > $kwsdatadir/proxy_set.list
  ...
  if [ ! -f $kwsdatadir/keywords_proxy.fsts ]; then
...
#   TODO should we do something special for short keywords which are not expanded?
    ...
    if $make_proxy_stochastic; then
      if ! $use_total_weight; then
        ...
        fstmakestochastic --total-weight=$proxy_log_weight ark:$kwsdatadir/keywords_proxy.fsts ark:$kwsdatadir/keywords_proxy_scaled.fsts
      else
        ## Method 2) if KWs only have proxy, assign total weight to them
        fstmakestochastic --total-weight=$total_log_weight ark:$kwsdatadir/keywords_proxy.fsts ark:$kwsdatadir/keywords_proxy_scaled.fsts
      fi
      touch $kwsdatadir/keywords_original_scaled.fsts
    else
      cp $kwsdatadir/keywords_proxy.fsts $kwsdatadir/keywords_proxy_scaled.fsts
...
EOF
    ...
    if false; then
      ...
      if [ `cat $kwsdatadir/no_proxy_set.list | wc -l` == 0 ]; then
        touch $kwsdatadir/keywords_original.int
      else
        grep -f $kwsdatadir/no_proxy_set.list $source_dir/keywords.int > $kwsdatadir/keywords_original.int
      fi
    else
    ...
    fstcopysubset --subset-key-file=$kwsdatadir/proxy_set.list ark:$kwsdatadir/keywords_proxy.fsts ark:$kwsdatadir/keywords_proxy_long.fsts
    fstcopysubset --subset-key-file=$kwsdatadir/no_proxy_set.list ark:$kwsdatadir/keywords_proxy.fsts ark:$kwsdatadir/keywords_proxy_short.fsts
    ...
    if $make_proxy_stochastic; then
      fstmakestochastic --total-weight=$proxy_log_weight ark:$kwsdatadir/keywords_proxy_long.fsts ark:$kwsdatadir/keywords_proxy_long_scaled.fsts
      ...
      fstmakestochastic --total-weight=$original_log_weight ark:$kwsdatadir/keywords_original_long.fsts ark:$kwsdatadir/keywords_original_long_scaled.fsts
      if ! $use_total_weight; then
        fstmakestochastic --total-weight=$original_log_weight ark:$kwsdatadir/keywords_original_short.fsts ark:$kwsdatadir/keywords_original_short_scaled.fsts
      else
        fstmakestochastic --total-weight=$total_log_weight ark:$kwsdatadir/keywords_original_short.fsts ark:$kwsdatadir/keywords_original_short_scaled.fsts
      fi
...
if [ "$dataset_kind" == "shadow" ]; then
  ...
  if [ ! -f $dataset_dir/.done.kws_ive3 ] ; then
    setup_proxy_search $dataset_dir $dataset_dir/kws false
    touch $dataset_dir/.done.kws_ive3
  fi
  if [ ! -f $dataset_dir/.done.oov_kws_ive3 ] ; then
    setup_proxy_search $dataset_dir $dataset_dir/oov_kws true
    touch $dataset_dir/.done.oov_kws_ive3
  fi
  if [ ${#my_more_kwlists[@]} -ne 0  ] ; then
    ...
    for extraid in "${!my_more_kwlists[@]}" ; do
      [ -f $dataset_dir/.done.kws_ive3.${extraid} ] && continue;
      setup_proxy_search $dataset_dir $dataset_dir/${extraid}_kws false $extraid
      touch $dataset_dir/.done.kws_ive3.${extraid}
    done
    for extraid in "${!my_more_kwlists[@]}" ; do
      [ -f $dataset_dir/.done.oov_kws_ive3.${extraid} ] && continue;
      setup_proxy_search $dataset_dir $dataset_dir/${extraid}_oov_kws true $extraid
      touch $dataset_dir/.done.oov_kws_ive3.${extraid}
    done
  fi
fi
