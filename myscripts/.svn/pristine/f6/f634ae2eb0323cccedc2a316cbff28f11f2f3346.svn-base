...
GetOptions("add=s" => \$nsWordsFile,  
           ...
           "phonemap=s" => \$phonemap,
           "vowel-list=s" => \$vowel_list,
           "stress-tag=s" => \$stress_tag
           );
...
if ($#ARGV == 1) {
    ...
    print STDERR ("prepare_lexicon: augmented version by chenzp (Feb 22,2014)");
    ...
    print STDERR ("\t--vowel-list      Append TAG only on vowels, which are defined by this\n");
    print STDERR ("\T                  list. If not set, TAG would be appended to all phonemes.\n");
...
#Process vowel-list, TAG would only be appended on them (chenzp, Feb 22,2014)
%is_vowel_hash;
@vowels = split(/ /, $vowel_list);
foreach (@vowels) {
    $is_vowel_hash{$_} = 1;
}
	 ...
	 ((!$romanized) && ($line =~ m:^([^\t]+)((\t[^\t]+)+)$:)) ) {
        ...
        for ($p=0; $p<=$#pron; ++$p) {
            ...
            while ($syllable = shift @syllables) {
                ...
                if (0 == @vowels) { # if vowel_list not defined, append sylTag to all phones.
                    $new_phones =~ s:(\S+):$1${sylTag}:g;
                } else {
                    @splitted = split(/ /, $new_phones);
                    $new_phones = "";
                    foreach $aphone(@splitted) {
                        if ($is_vowel_hash{$aphone} == 1) {
                            $aphone =~ s:$:${sylTag}:;
                        }
                        $new_phones .= " $aphone";
                    }
