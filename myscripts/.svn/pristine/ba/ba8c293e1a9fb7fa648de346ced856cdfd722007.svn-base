...
if [ ! -f data/train_seg/.done ]; then
  ...
  echo ---------------------------------------------------------------------
  czpScripts/local/prepare_acoustic_training_data.chenzp.pl --get-whole-transcripts "true" \
...
czpScripts/local/resegment/train_segmentation.chenzp.sh \
  --boost-sil $boost_sil --nj $train_nj --cmd "$decode_cmd" \
  --num-leaves $num_leaves --num-gauss $num_gauss \
