...
if [ -z $extraid ] ; then
  kwsdatadir=$1/$basekwsdir
else
  kwsdatadir=$1/${extraid}_$basekwsdir
fi
kwsoutputdir="$2/"
...
czpScripts/local/kws_oracle_threshold.chenzp.pl --fast $fast --duration $duration ${kwsoutputdir}${f4de_prefix}/alignment.csv > ${kwsoutputdir}${f4de_prefix}/metrics.txt
