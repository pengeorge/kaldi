...
if [ ! -z $existing_proxy_fsts ]; then 
  if [ ! -f $existing_proxy_fsts ]; then
    echo "existing_proxy_fsts ($existing_proxy_fsts) does not exist"
    exit 1
  fi
  fsttablesubset.chenzp "ark:$existing_proxy_fsts" <(cut -f 1 $kwsdatadir/tmp/keywords.txt) \
    "ark:$kwsdatadir/keywords.fsts"
else
