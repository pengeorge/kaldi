...
GetOptions("fragmentMarkers=s" => \$fragMarkers,
           ...
           "case-insensitive=s" => \$case_insensitive
           );
...
if (-d $TranscriptionDir) {
    @TranscriptionFiles = `find ${TranscriptionDir} -name '*.txt'`;
        ...
        while ($filename = shift @TranscriptionFiles) {
            ...
            while ($line=<TRANSCRIPT>) {
                ...
                if ($line =~ m:^\s*\[([0-9]+\.*[0-9]*)\]\s*$:) {
                    ...
                    $prevTimeMark = $thisTimeMark;
                  } elsif ($line eq "<untranscribed>") {
                    # do nothing
                    if ($get_whole_transcripts eq "true") { # VAD case
                      $text = "$silence"; # in VLLP transcripts, untranscribed seems to be always <silence>
                    }
                } else {
		    @tokens = split(/\s+/, $line);
		    $text = "";
		    while ($w = shift(@tokens)) {
        if ($case_insensitive eq "true") {
          $w = encode('utf8', lc(decode('utf8',$w)));
        }
...
if (-d $AudioDir) {
    @AudioFiles = `find ${AudioDir} -name '*.sph'`;
    if ($#AudioFiles >= 0) {
    ...
    @AudioFiles = `find ${AudioDir} -name '*.wav'`;
    if ($#AudioFiles >= 0) {
