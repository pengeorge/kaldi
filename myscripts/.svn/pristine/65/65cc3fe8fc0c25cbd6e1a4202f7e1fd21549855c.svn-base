#!/bin/bash
# Set no_2nd_lda to train a multilang NN with or without a second LDA (calling get_lda.sh or not)
# TODO I guess "no LDA in NN" may be more reasonalbe in multilang scenario.
...
if $no_2nd_lda; then
  initdir=${initdir}.no_2nd_lda
  mlsuffix=${mlsuffix}.no_2nd_lda
  init_model_recipe=./czpScripts/nnet2/train_pnorm_fast.no_2nd_lda.sh
else
  init_model_recipe=./steps/nnet2/train_pnorm_fast.sh
fi
