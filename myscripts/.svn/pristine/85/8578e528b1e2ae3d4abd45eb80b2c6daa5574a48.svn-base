...
if [ -z $nbest0 ]; then
  nbest0=$proxy_nbest0  # if not defined, read from lang.conf
fi
...
if $lm_in_expansion; then
  suffix=${suffix}-lm
  if [ ! -z $nbest0 ] && [ $nbest0 != '-1' ]; then
    suffix=${suffix}-${nbest0}
  fi
fi
...
function setup_proxy_search {
  ...
  if $lm_in_expansion; then
    proxy_dir=${proxy_dir}-lm
    if [ ! -z $nbest0 ] && [ $nbest0 != '-1' ]; then
      proxy_dir=${proxy_dir}-${nbest0}
    fi
  fi
  ...
  if [ ! -f $proxy_dir/.done ]; then # here we need to generate proxies
    ...
    if [ ! -f exp/conf_matrix/$model4cm/.done ] ; then
      echo "Now train a confusion matrix using $model4cm and ${graph_model4cm}'s graph"
      if [ ! -d exp/$graph_model4cm/graph ]; then
        utils/mkgraph.sh \
          $lang_dir exp/$graph_model4cm exp/$graph_model4cm/graph |tee exp/$graph_model4cm/mkgraph.log
      fi
      local/generate_confusion_matrix.sh --cmd "$decode_cmd" --nj $my_nj  \
        exp/$graph_model4cm/graph exp/$model4cm exp/${model4cm}_ali exp/${model4cm}_denlats  exp/conf_matrix/$model4cm
      touch exp/conf_matrix/$model4cm/.done 
    fi
    confusion=exp/conf_matrix/$model4cm/confusions.txt
  ...
  #    grep -v -f <(cut -f 1 $data_dir/${extraid}oov_kws/keywords.txt) $data_dir/${extraid}kws/keywords.txt | cut -f 1 > $kwsdatadir/proxy_set.list
      ...
      if $case_insensitive; then
        paste <(cut -f 1 -d ' ' $data_dir/${extraid}oov_kws/tmp/L1.lex |\
          ...
          sed 's: \+: :g' | sort -u > $proxy_dir/L2_from_L1.lex # in IV case, L2 is the same as L1 but all lower-case
      else
        cp $data_dir/${extraid}oov_kws/tmp/L1.lex $proxy_dir/L2_from_L1.lex
      fi
      L2_lex=$proxy_dir/L2_from_L1.lex 
    fi
    ...
    local/kws_data_prep_proxy.chenzp.sh \
      ...
      --phone-cutoff 1 \
      --pron-probs true --beam $beam --nbest $nbest --nbest0 $nbest0 \
      ...
      $lang_dir  $data_dir $L1_lex $L2_lex $proxy_dir
...
#   TODO should we do something special for short keywords which are not expanded?
    ...
    cat > $kwsdatadir/conf << EOF
    make_proxy_stochastic=$make_proxy_stochastic
    iv_phone_cutoff=$iv_phone_cutoff
    proxy_weight=$proxy_weight
...
EOF
    ...
    if [ -z $silence_word ]; then
      ...
      silence_int=`grep -w $silence_word $lang_dir/words.txt | awk '{print $2}'`
      [ -z $silence_int ] && \
