...
if [ $# != 5 ]; then
  echo "Usage: $0 [opts] <data> <lang> <ali-dir> <input-model> <exp-dir>"
  echo " e.g.: $0 data/train data/lang exp/tri3_ali exp/tri4_nnet/final.mdl exp/tri4_nnet_2"
  echo ""
...
if [ $stage -le -3 ] && [ -z "$egs_dir" ]; then
  ...
  czpScripts/nnet2/get_egs.get_feat_dim.sh --samples-per-iter $samples_per_iter --num-jobs-nnet $num_jobs_nnet \
      --feat-type "$feat_type" --splice-width $splice_width \
      --transform-dir "$transform_dir" \
      --stage $get_egs_stage --cmd "$cmd" $egs_opts --io-opts "$io_opts" \
...
# Reinitialization the last layer ($ensemble_size times)
echo $input_model > $dir/input_model
$cmd $parallel_opts JOB=1:$ensemble_size $dir/log/reinitialize.JOB.log \
  nnet-am-reinitialize $input_model $alidir/final.mdl $dir/0.JOB.mdl || exit 1;
