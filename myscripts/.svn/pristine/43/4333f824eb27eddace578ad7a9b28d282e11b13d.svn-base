...
function setup_proxy_search {
  ...
  local kwsdatadir=${source_dir}_ive2
  mkdir -p $kwsdatadir
...
if [ "$dataset_kind" == "shadow" ]; then
  ...
  if [ ! -f $dataset_dir/.done.kws_ive2 ] ; then
    setup_proxy_search $dataset_dir $dataset_dir/kws false
    touch $dataset_dir/.done.kws_ive2
  fi
  if [ ! -f $dataset_dir/.done.oov_kws_ive2 ] ; then
    setup_proxy_search $dataset_dir $dataset_dir/oov_kws true
    touch $dataset_dir/.done.oov_kws_ive2
  fi
  if [ ${#my_more_kwlists[@]} -ne 0  ] ; then
    ...
    for extraid in "${!my_more_kwlists[@]}" ; do
      [ -f $dataset_dir/.done.kws_ive2.${extraid} ] && continue;
      setup_proxy_search $dataset_dir $dataset_dir/${extraid}_kws false $extraid
      touch $dataset_dir/.done.kws_ive2.${extraid}
    done
    for extraid in "${!my_more_kwlists[@]}" ; do
      [ -f $dataset_dir/.done.oov_kws_ive2.${extraid} ] && continue;
      setup_proxy_search $dataset_dir $dataset_dir/${extraid}_oov_kws true $extraid
      touch $dataset_dir/.done.oov_kws_ive2.${extraid}
    done
  fi
fi
