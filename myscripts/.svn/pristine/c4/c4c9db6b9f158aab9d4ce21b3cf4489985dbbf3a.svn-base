...
if [ "$nbest" -eq 0 ]; then
  ...
  if $use_log; then
    if $self_prior; then
      ...
      exit 1;
    elif $lm_in_expansion; then
      echo "The script doesn't support generating proxy keywords using log semring while considering LM scores."
      exit 1;
    else
    ...
    if $lm_in_expansion; then
      # $kwsdatadir is like data/dev/proxy/tmp
      langdir=$kwsdatadir/../../../lang
      if [ ! -f $langdir/.done.reciprocalG ]; then
        assist_sym_id=`grep -Po '(?<=^#0 )\d+$' $langdir/words.txt`
        eps_id=`grep -Po '(?<=^<eps> )\d+$' $langdir/words.txt`
        fstreciprocalweights $langdir/G.fst | fstarcsort --sort_type=ilabel | \
          fstprint | awk -F"\t" '{if($3=='$assist_sym_id'){ $3='$eps_id'; } if($4=='$assist_sym_id'){ $4='$eps_id'; }print $0;}' | fstcompile > $langdir/reciprocalG.fst
        touch $langdir/.done.reciprocalG
      fi
      lm_opt="--additional-fst=$langdir/reciprocalG.fst"
    else
      lm_opt=
    fi
    if $self_prior; then
  ...
  $cmd JOB=1:$nj $kwsdatadir/split/log/proxy.JOB.log \
    ...
    --proxy-beam=$beam --proxy-nbest=$nbest --proxy-nbest0=$nbest0 \
    --phone-beam=$phone_beam --phone-nbest=$phone_nbest \
    $lm_opt $prior_opt \
    $kwsdatadir/L2xE.fst $kwsdatadir/L1.fst ark:- ark:$kwsdatadir/split/proxy.JOB.fsts ark,t:$kwsdatadir/split/proxy.kwlist.JOB.txt ';'\
