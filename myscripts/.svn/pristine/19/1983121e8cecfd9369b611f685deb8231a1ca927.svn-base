...
# We permit the absence of word-boundary.int, which indicates the phone set is not position dependency
# (chenzp Feb 23,2014)
...
if [ $stage -le 0 ]; then
  if [ -f $lang/phones/word_boundary.int ]; then # if word_boundary exists, do lattice-align-words
  ...
  else # if word_boundary doesn't exist, skip lattice-align-words
  $cmd LMWT=$min_lmwt:$max_lmwt $dir/scoring/log/get_ctm.LMWT.log \
    set -e -o pipefail \; \
    mkdir -p $dir/score_LMWT/ '&&' \
    lattice-scale --inv-acoustic-scale=LMWT "ark:gunzip -c $dir/lat.*.gz|" ark:- \| \
