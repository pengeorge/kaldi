...
if [ -z $kwsout_dir ] ; then
  if [ -z $extraid ] ; then
    ...
    newoutdir=$decodedir/norm-$method/$basekwsdir
  else
    kwsoutdir=$decodedir/${extraid}_$basekwsdir
    newoutdir=$decodedir/norm-$method/${extraid}_$basekwsdir
  fi
else
...
if [ $stage -le 1 ]; then
  ...
  $cmd LMWT=$min_lmwt:$max_lmwt ${newoutdir}/write.LMWT.log \
    ...
    cat ${kwsoutdir}_LMWT/result.* \| \
      czpScripts/kws/write_kwslist.chenzp.pl  --Ntrue-scale=$ntrue_scale --flen=0.01 --duration=$duration \
        --segments=$datadir/segments --normalize=$method --duptime=$duptime --remove-dup=true\
        --map-utter=$kwsdatadir/utter_map $flag \
        - ${newoutdir}_LMWT/kwslist.xml || exit 1
fi
