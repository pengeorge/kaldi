...
utter_id=$datadir/kws_common/utter_id
...
for f in $model $decodedir/lat.1.gz; do
...
# Try to index without word_boundary file, as phone set may not be position dependency (chenzp, Jan 6,2014)
# But can this indexing work normally ?
$cmd JOB=1:$nj $kwsdir/log/index.JOB.log \
    lattice-add-penalty --word-ins-penalty=$word_ins_penalty "ark:gzip -cdf $decodedir/lat.JOB.gz|" ark:- \| \
    lattice-scale --acoustic-scale=$acwt --lm-scale=$lmwt ark:- ark:- \| \
    lattice-to-kws-index --max-states-scale=$max_states_scale --allow-partial=true \
...
#$cmd JOB=1:$nj $kwsdir/log/index_ap.JOB.log \
#  lattice-add-penalty --word-ins-penalty=$word_ins_penalty "ark:gzip -cdf $decodedir/lat.JOB.gz|" ark:$kwsdir/lat-ap.JOB
#$cmd JOB=1:$nj $kwsdir/log/index_scale.JOB.log \
#    lattice-scale --acoustic-scale=$acwt --lm-scale=$lmwt ark:$kwsdir/lat-ap.JOB ark:$kwsdir/lat-scale.JOB
#$cmd JOB=1:$nj $kwsdir/log/index_lat2idx.JOB.log \
#    lattice-to-kws-index --verbose=2 --max-silence-frames=$max_silence_frames --strict=$strict ark:$utter_id ark:$kwsdir/lat-scale.JOB ark:$kwsdir/lat-idx.JOB
#$cmd JOB=1:$nj $kwsdir/log/index_union.JOB.log \
#    kws-index-union --skip-optimization=$skip_optimization --strict=$strict --max-states=$max_states \
#    ark:$kwsdir/lat-idx.JOB "ark:|gzip -c > $kwsdir/index.JOB.gz"
fi
