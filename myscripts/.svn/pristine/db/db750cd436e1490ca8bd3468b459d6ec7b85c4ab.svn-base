...
foreach my $key (keys %ins) {
  $ins{$key} = -log($ins{$key}/($subs_count+$del_count));
}
foreach my $key (keys %del) {
  $del{$key} = -log($del{$key}/($phone_count{$key}+$del{$key}));
}
foreach my $key (keys %subs) {
  my @col = split(/_/, $key);
  if (!defined($del{$col[0]})) {
    $del{$col[0]} = 0;
  }
  $subs{$key} = -log($subs{$key}/($phone_count{$col[0]}+$del{$col[0]}));
}
