...
if $check_phone_set; then
  echo "Please check phoneme set before continuing: y/n"
  read
  if [[ ! "$REPLY" =~ ^y ]]; then
...
if $case_insensitive; then
  icu_opt=" --icu-transform Any-Lower "
else
  icu_opt=
fi
...
if [ ! -f exp/sgmm5/.done ]; then
  ...
  echo ---------------------------------------------------------------------
  czpScripts/steps/train_sgmm2.chenzp.sh \
