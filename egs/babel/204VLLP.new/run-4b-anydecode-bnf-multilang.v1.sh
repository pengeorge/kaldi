...
feat_type=raw # type of BNF input features
unsup_string=
...
dirid=${type}
exp_dir=exp_bnf_${mlsuffix}${unsup_string}
data_bnf_dir=data_bnf_${mlsuffix}${unsup_string}
param_bnf_dir=param_bnf_${mlsuffix}${unsup_string}
...
if [ ! $data_bnf_dir/${dirid}_bnf/.done -nt exp/tri5/decode_${dirid}/.done ] || \
  ...
  czpScripts/nnet2/dump_bottleneck_features.chenzp.sh --nj $my_nj --cmd "$train_cmd" \
    --feat-type $feat_type $transform_dir_opts data/${dirid} $data_bnf_dir/${dirid}_bnf $exp_dir/tri6_bnf $param_bnf_dir $exp_dir/dump_bnf
