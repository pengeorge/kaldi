...
if [ ! $exp_dir/tri6_ali/.done -nt $exp_dir/tri6/.done ]; then
  echo ---------------------------------------------------------------------
  echo "Aligning fMLLR system with $train_nj jobs"
  echo ---------------------------------------------------------------------
  steps/align_fmllr.sh \
    --boost-silence $boost_sil --nj $train_nj --cmd "$train_cmd" \
    $data_bnf_dir/train data/lang $exp_dir/tri6 $exp_dir/tri6_ali
  touch $exp_dir/tri6_ali/.done
fi


if [ ! $exp_dir/tri7_nnet/.done -nt $exp_dir/tri6_ali/.done ]; then
  echo ---------------------------------------------------------------------
  ...
  steps/nnet2/train_pnorm.sh \
    ...
    $data_bnf_dir/train data/lang $exp_dir/tri6_ali $exp_dir/tri7_nnet || exit 1 
...
echo "To decode a data-set, use run-4b-anydecode-bnf-multilang.sh"
echo ---------------------------------------------------------------------
