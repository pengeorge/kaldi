...
datadir=data/${dirid}
exp_dir=exp_bnf_mrasta${unsup_string}
data_bnf_dir=data_bnf_mrasta${unsup_string}
param_bnf_dir=param_bnf_mrasta${unsup_string}
...
if [ ! -f $hi_exp_dir/tri6_bnf/.done ]; then    
 ...
 echo "$0: Train 1st Bottleneck network"
  ...
  czpScripts/nnet2/train_tanh_bottleneck.chenzp.sh \
    ...
    --egs-opts " --feat-type raw " --splice-width 0 --do-lda false \
...
if [ ! $hi_data_bnf_dir/train/.done -nt $hi_data_bnf_dir/train_bnf/.done ]; then
  ...
  for f in segments spk2utt utt2spk wav.scp text reco2file_and_channel; do
    cp $hi_data_bnf_dir/train_bnf/$f $hi_data_bnf_dir/train/
  done
  #TODO LDA/PCA
  splice_opts=" --left-context=$mrasta_bnf1_splice_width --right-context=$mrasta_bnf1_splice_width "
  echo $splice_opts > $hi_data_bnf_dir/bnf1_splice_opts
  czpScripts/steps/append_feats_ext.sh --cmd "$train_cmd" --nj $train_nj \
    "ark,s,cs:splice-feats $splice_opts scp:$hi_data_bnf_dir/train_bnf/split${train_nj}/JOB/feats.scp ark:- |" "scp:data_mrasta.lo/train/split${train_nj}/JOB/feats.scp" $hi_data_bnf_dir/train \
