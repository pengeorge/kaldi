...
if [ ! -f ${dataset_dir}/kws/.done ] ; then
  if [ "$dataset_kind" == "shadow" ]; then
    ...
    if [ ! -f /home/chenty/kaldi-exp/viet-phone-notone/data/${dev2shadow}/kws/.done ]; then
      echo "Error: /home/chenty/kaldi-exp/viet-phone-notone/data/${dev2shadow}/kws/.done does not exist."
      echo "Create the directory /home/chenty/kaldi-exp/viet-phone-notone/data/${dev2shadow} first, by calling $0 --dir $dev2shadow --dataonly"
      exit 1
    fi
    if [ ! -f /home/chenty/kaldi-exp/viet-phone-notone/data/${eval2shadow}/kws/.done ]; then
      echo "Error: /home/chenty/kaldi-exp/viet-phone-notone/data/${eval2shadow}/kws/.done does not exist."
      echo "Create the directory /home/chenty/kaldi-exp/viet-phone-notone/data/${eval2shadow} first, by calling $0 --dir $eval2shadow --dataonly"
      exit 1
    fi

    local/kws_data_prep.sh --case_insensitive $case_insensitive \
      "${icu_opt[@]}" \
      /home/chenty/kaldi-exp/viet-phone-notone/data/lang ${dataset_dir} ${datadir}/kws || exit 1
    ...
    local/kws_setup.sh --case_insensitive $case_insensitive \
      "${kws_flags[@]}" "${icu_opt[@]}" \
      $my_ecf_file $my_kwlist_file /home/chenty/kaldi-exp/viet-phone-notone/data/lang ${dataset_dir} || exit 1
    set -u
  fi
  touch ${dataset_dir}/kws/.done 
fi
