...
echo --------------------------
echo extra_kws_subword.chenzp.sh
echo --------------------------
if [ -z $model4cm ]; then
  model4cm=sgmm5
...
function setup_oov_search_subword {
  ...
  mkdir -p $kwsdatadir
  for f in $source_dir/kwlist*.xml $source_dir/ecf.xml; do
  ...
  if ! $is_graphemic_lex; then
    ...
    local/apply_g2p.chenzp.sh --nj $my_nj --cmd "$decode_cmd" \
      ...
      --ext-pron "$ext_pron" \
      $kwsdatadir/oov.txt $word_system_root/exp/g2p${ext_word_lm_suffix} $kwsdatadir/g2p
  ...
  local/kws_data_prep_proxy_subword.chenzp.sh \
    ...
    $Lw2p_lex $L1_lex $L2_lex $kwsdatadir
...
if [ "$dataset_kind" == "shadow" ]; then
  ...
  if $basic_kws && $oov_kws && [ ! -f $dataset_dir/.done.kws.oov ] && [ ! -f $dataset_dir/oov_kws/.done ]; then
    setup_oov_search_subword $dataset_dir $dataset_dir/kws oov
  ...
  if [ ${#my_more_kwlists[@]} -ne 0  ] ; then
    ...
    for extraid in "${!my_more_kwlists[@]}" ; do
      ...
      local/kws_setup_subword.chenzp.sh  --extraid $extraid --case_insensitive $case_insensitive \
        "${kws_flags[@]}" "${icu_opt[@]}" \
        $my_ecf_file $kwlist \
        $w2sfile \
      ...
      #touch $dataset_dir/.done.kws.$extraid # only use .done in kws dir
    ...
    if $oov_kws; then
      for extraid in "${!my_more_kwlists[@]}" ; do
        ...
        #know in which directory the KWS setup will reside in, so we will 
        #place  the .done file directly into the data directory
        ([ -f $dataset_dir/.done.kws.${extraid}_oov ] || [ -f $dataset_dir/${extraid}_oov_kws/.done ]) && continue;
        setup_oov_search_subword $dataset_dir $dataset_dir/${extraid}_kws ${extraid}_oov
        register_extraid $dataset_dir ${extraid}_oov
        #touch $dataset_dir/.done.kws.${extraid}_oov
        touch $dataset_dir/${extraid}_oov_kws/.done
      done
    fi
  fi
fi
