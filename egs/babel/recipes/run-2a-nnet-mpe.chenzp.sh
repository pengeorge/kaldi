...
echo "Waiting till exp/tri6b_nnet/.done exists...."
while [ ! -f exp/tri6b_nnet/.done ]; do sleep 30; done
echo "...done waiting for exp/tri6b_nnet/.done"

# Generate denominator lattices.
if [ ! -f exp/tri6_nnet_denlats$ext/.done ]; then
  steps/nnet2/make_denlats.chenzp.sh "${dnn_denlats_extra_opts[@]}" \
    --nj $train_nj --sub-split $sub_split \
    --transform-dir exp/tri5_ali \
    data/train data/lang$ext exp/tri6b_nnet exp/tri6_nnet_denlats$ext || exit 1
 
  touch exp/tri6_nnet_denlats$ext/.done
fi
...
if [ ! -f exp/tri6_nnet_ali/.done ]; then
  steps/nnet2/align.sh --use-gpu yes \
    ...
    data/train data/lang$ext exp/tri6b_nnet exp/tri6_nnet_ali || exit 1

  touch exp/tri6_nnet_ali/.done
fi

if [ ! -f exp/tri6_nnet_mpe$ext/.done ]; then
  steps/nnet2/train_discriminative.sh \
    ...
    exp/tri6_nnet_ali exp/tri6_nnet_denlats$ext exp/tri6b_nnet/final.mdl exp/tri6_nnet_mpe$ext || exit 1

  touch exp/tri6_nnet_mpe$ext/.done
fi
